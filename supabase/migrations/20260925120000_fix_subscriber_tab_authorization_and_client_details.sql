...
begin
  ...
  if not (public.user_has_permission(auth.uid(), 'admin:view_subscribers') or public.user_has_permission(auth.uid(), 'admin:all')) then
  ...
  select
    ...
    coalesce(nullif(trim(concat_ws(' ', p.first_name, p.last_name)), ''), p.full_name, 'Unnamed Subscriber')::text,
    es.estate_name,
    es.estate_code,
    es.user_id,
    es.order_id,
    ...
    greatest(1, coalesce((select sum(coalesce((x->>'quantity')::integer, 1)) from jsonb_array_elements(coalesce(o.items, '[]'::jsonb)) x), 1))::integer,
    o.total_amount,
    o.amount_paid,
  ...
  where (
    ...
    or es.subscription_number ilike '%' || _search || '%'
    or es.estate_name ilike '%' || _search || '%'
    or coalesce(p.email, u.email) ilike '%' || _search || '%'
    or coalesce(p.full_name, concat_ws(' ', p.first_name, p.last_name)) ilike '%' || _search || '%'
  )
    and (_estate_code is null or _estate_code = '' or es.estate_code = _estate_code)
...
begin
  ...
  select
    ...
    oi.amount_paid,
    oi.amount_due,
    case when lower(coalesce(pr.type, '')) like '%documentation%' then coalesce(pr.description, 'Documentation Bundle') else null end
  from public.payment_requests pr
  left join public.order_installments oi on oi.payment_id = pr.related_payment_id and oi.order_id = _order_id
  where pr.user_id = (select user_id from public.orders where id = _order_id)
    and (
      pr.description ilike '%' || (select coalesce(customer_name, '') from public.orders where id = _order_id) || '%'
      or pr.related_payment_id in (select oi2.payment_id from public.order_installments oi2 where oi2.order_id = _order_id)
      or pr.reference = (select payment_reference from public.orders where id = _order_id)
    )
  order by coalesce(pr.processed_at, pr.created_at) desc;
end;
$function$;
