...
begin
 if caller is null or not (public.user_has_permission(caller,'admin:approve_admin_requests') or public.user_has_permission(caller,'admin:manage_users') or public.user_has_permission(caller,'admin:manage_departments') or public.user_has_permission(caller,'admin:all')) then
   raise exception using errcode='42501',message='Not authorized to approve admin requests';
 end if;
...
begin
 if caller is null or not (public.user_has_permission(caller,'admin:approve_payments') or public.user_has_permission(caller,'admin:all')) then
   raise exception using errcode='42501',message='Not authorized to approve payments';
 end if;
