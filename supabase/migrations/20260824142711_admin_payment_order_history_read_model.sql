...
begin
 ...
 if not (public.has_permission(uid,'admin:approve_payments') or public.has_permission(uid,'admin:all')) then raise exception using errcode='42501',message='Not authorized to view payment approval details'; end if;
 ...
 left join orders o on o.id=coalesce(p.order_id,pr.order_id)
