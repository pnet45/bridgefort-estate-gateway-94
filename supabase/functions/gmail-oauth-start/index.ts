...
serve(async (req) => {
  ...
  try {
    ...
    if (!mailboxEmail) return corsJson(req, { error: "mailboxEmail is required" }, 400);
    ...
    if (!access) return corsJson(req, { error: "Forbidden: mailbox access denied" }, 403);
    ...
    if (!assignments?.length) return corsJson(req, { error: "Gmail mailbox assignment not found" }, 404);
    ...
    if (!assignedAccounts.length) return corsJson(req, { error: "No Google account has been assigned to this mailbox" }, 400);
    ...
    return corsJson(req, { url: `https://accounts.google.com/o/oauth2/v2/auth?${params.toString()}`, mailboxEmail, assignedAccounts });
  } catch (error: any) {
    console.error("gmail-oauth-start error:", error);
    return corsJson(req, { error: error?.message || "Unable to start Gmail connection" }, 500);
  }
});
