...
const handler = async (req: Request): Promise<Response> => {
  ...
  // Restrict to internal service-role callers only (typically invoked by
  // another edge function or trigger, not directly from the browser).
  ...
  try {
    ...
    if (!lockedEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(lockedEmail)) {
      return new Response(JSON.stringify({ error: "Invalid email" }), {
        status: 400, headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }
    const safeEmail = escapeHtml(lockedEmail);
    const safeAttempt = Number.isFinite(Number(attemptCount)) ? Number(attemptCount) : 0;
          ...
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="details">
                ...
                <p><strong>Email:</strong> <code>${safeEmail}</code></p>
                <p><strong>Failed Attempts:</strong> ${safeAttempt}</p>
              ...
              <div class="details">
                ...
                <code>SELECT clear_failed_logins('${safeEmail}');</code>
              </div>
