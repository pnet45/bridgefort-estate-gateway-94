...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: deleteError } = await svc.from("gmail_oauth_tokens").delete().eq("email", mailbox);
    ...
    return new Response(JSON.stringify({ success: true, mailboxEmail: mailbox }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error: any) {
