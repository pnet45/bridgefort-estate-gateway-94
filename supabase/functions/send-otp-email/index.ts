...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    // This endpoint is not for public use — request-password-reset owns OTP
    // generation. Restrict to internal service-role callers only.
    ...
    if (!email || !otp || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || !/^\d{4,8}$/.test(otp)) {
