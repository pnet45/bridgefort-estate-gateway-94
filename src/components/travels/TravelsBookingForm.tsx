...
const TravelsBookingForm: React.FC<Props> = ({
  initialPackage = '',
  initialDestination = '',
}) => {
  ...
  // ============================================================
  // UPDATE INITIAL PACKAGE / DESTINATION
  // ============================================================
  ...
  // ============================================================
  // PACKAGE AVAILABILITY / BLACKOUT CHECK
  // ============================================================

  const availabilityWarning = useMemo(() => {
    if (!form.package || !form.departure || !form.return) {
      return null;
    }
    ...
    return `The ${form.package} package isn't available between ${conflict.start_date} and ${conflict.end_date}${
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      new Date(result.data.return) <
      new Date(result.data.departure)
    ) {
      setErrors({
    ...
    // ==========================================================
    //
    // The following check is intentionally disabled.
    //
    // if (!captchaToken) {
    //   toast({
    ...
    try {
      ...
      // --------------------------------------------------------
      // HANDLE APPLICATION ERROR RETURNED BY EDGE FUNCTION
      // --------------------------------------------------------
      ...
      toast({
        ...
        description:
          "We've emailed you a confirmation. Redirecting to your booking status…",
      });

      // --------------------------------------------------------
      // CONFIRMATION TOKEN REDIRECT
      // --------------------------------------------------------

      if (data?.token) {
        setTimeout(() => {
          navigate(`/travels/booking/${data.token}`);
        }, 1200);
      }
    } catch (err: any) {
      // ========================================================
  ...
  return (
    <section
      id="booking"
      className="section-padding bg-gradient-to-br from-muted/40 to-background"
    >
          ...
          {submitted ? (
            <div className="text-center py-10">
              ...
              <p className="text-muted-foreground mb-6">
                Your enquiry has been received. A Bridgefort travel consultant will contact you at{' '}
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Visa Only">
                      Visa assistance only
                    </SelectItem>

                    <SelectItem value="Flight Only">
                      Flight booking only
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              {availabilityWarning && (
                <div className="md:col-span-2 flex items-start gap-2 rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">
                  ...
                  <span>
                    {availabilityWarning}{' '}
                    Please pick different dates or choose another package.
                  </span>
                </div>
              )}
