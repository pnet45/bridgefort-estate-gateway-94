...
const PropertyDetailsDialogFullscreen = ({
  ...
  useEffect(() => {
    if (isOpen && property.id) {
      // Fetch estate data from DB for description
      supabase.from('estate').select('*').eq('id', property.id).single().then(({ data }) => {
        if (data) setEstateData(data);
      });
    }
  }, [isOpen, property.id]);
  ...
  const checkProfileCompletion = async () => {
    ...
    } catch (error) { console.error('Error checking profile completion:', error); }
  };
  ...
  const handleAddToCart = () => {
    if (!user) { toast({ title: "Login Required", description: "Please login to add properties to cart.", variant: "destructive" }); return; }
    if (!profileCompleted) { toast({ title: "Profile Incomplete", description: "Please complete your profile before purchasing properties.", variant: "destructive" }); return; }
    const plot: Plot = {
  ...
  const getPropertyPDF = (propertyTitle: string) => {
    switch (propertyTitle) {
      ...
      default: return `/lovable-uploads/2025-CURRENT-SUB-FORM-FORTRESS-HILLS-IKORODU-PHASE-1-&-2.pdf`;
    }
  };

  const getPropertyImages = (propertyTitle: string) => {
    // Use media from estate data if available
    if (estateData?.media && estateData.media.length > 0) return estateData.media;
    switch (propertyTitle) {
      case "Akuchi Luxury Estate": return ["/lovable-uploads/fbb073b9-ed7e-4a53-bcb5-54c870b10b6f.png"];
      case "Fortress Hills Estate": return ["/lovable-uploads/b6b178d0-ae26-4527-9569-dce064d705b9.png"];
      default: return [property.imageUrl];
    }
  };
  ...
  const propertyDescription = estateData?.description || property.description || '';
          ...
          <div className="flex flex-col h-[95vh]">
            ...
            <div className="flex items-center justify-between p-6 border-b bg-estate-blue text-white">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-blue-100 flex items-center"><MapPin size={16} className="mr-1" />{property.location}</p>
                </div>
              </div>
              <Button variant="ghost" size="icon" onClick={onClose} className="text-white hover:bg-white/20"><X size={24} /></Button>
            </div>
              ...
              <ScrollArea className="h-full">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
                  ...
                  <div className="space-y-6">
                    ...
                    {/* Property Overview with Description */}
                    <Card>
                      <CardHeader>
                  ...
                  <div className="space-y-6">
                    {estateDetails ? (
                      ...
                      <Card>
                        ...
                        <CardContent>
                          {propertyDescription ? (
                            <p className="text-sm whitespace-pre-wrap">{propertyDescription}</p>
                          ) : (
            ...
            <div className="border-t p-6 bg-gray-50">
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button onClick={handleDownload} className="bg-estate-blue hover:bg-estate-darkBlue text-white px-8 py-3">
                  <Download className="mr-2" size={16} /> Download Subscription Form
                </Button>
                <Button onClick={handleAddToCart} className="bg-estate-red hover:bg-red-600 text-white px-8 py-3">
