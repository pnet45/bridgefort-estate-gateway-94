...
const AdminDepartmentManagement = () => {
  ...
  const reset = () => {
    setEditing(null); setName(''); setSlug(''); setDescription(''); setIsActive(true);
  };
  ...
  const save = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      role_name: `admin_${slug.trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '')}`,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div><h2 className="text-xl font-semibold">Departments</h2><p className="text-sm text-muted-foreground">Create administrator spaces and make them available for future admin assignments.</p></div>
      </div>
      ...
      <form onSubmit={save} className="rounded-lg border p-5 space-y-4 bg-card">
        ...
        <div className="flex items-center justify-between"><div><Label>Available for new administrators</Label><p className="text-xs text-muted-foreground">Inactive departments cannot be selected during admin registration.</p></div><Switch checked={isActive} onCheckedChange={setIsActive} /></div>
