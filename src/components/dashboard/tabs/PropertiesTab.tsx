...
const PropertiesTab = () => {
  ...
  const loadProperties = async () => {
    ...
    const entries = await Promise.all((data || []).map(async (p: any) => {
      const { data: events } = await supabase.rpc('get_my_property_payment_timeline', { _order_id: p.order_id });
      return [p.order_id, events || []] as const;
    }));
  ...
  const grouped = useMemo(() => properties.map((p) => {
    ...
    return { ...p, balance: Math.max(total - paid, 0), plotCount: Math.max(1, asNumber(p.quantity) || (p.plot_id ? String(p.plot_id).split(',').map((x: string) => x.trim()).filter(Boolean).length : 1)) };
  }), [properties]);
  ...
  const startDocumentationPayment = async (property: any) => {
    ...
    try {
      ...
      const { data: sessionData } = await supabase.auth.getSession(); const email = sessionData.session?.user?.email;
      if (!email) throw new Error('Your account email could not be found.');
      const { data, error: fnError } = await supabase.functions.invoke('paystack-initialize', { body: { email, order_id: property.order_id, payment_id: paymentId, amount: checkout.amount, payment_type: 'documentation', metadata: { order_id: property.order_id, payment_id: paymentId, payment_type: 'documentation', documentation_type_id: checkout.documentation_type_id, documentation_name: checkout.documentation_name, property_name: checkout.property_name } } });
      if (fnError) throw fnError; const checkoutUrl = data?.data?.authorization_url || data?.authorization_url || data?.data?.checkout_url || data?.checkout_url;
  ...
  return <div className="space-y-6">
    <Card className="overflow-hidden border-white/10 bg-background/70 backdrop-blur-xl shadow-xl">
      <CardHeader className="border-b border-white/10 bg-white/[0.03]"><CardTitle className="flex items-center gap-2"><Building size={20} /> My Properties</CardTitle><p className="text-sm text-muted-foreground">Track your estate purchases, plots, documentation, payments and progress in one place.</p></CardHeader>
      <CardContent className="p-4 md:p-6 space-y-5">
        ...
        {!grouped.length ? <div className="rounded-2xl border border-dashed border-white/10 py-14 text-center text-muted-foreground">No purchased property found yet. Start browsing available properties to make your first purchase.</div> : grouped.map((p) => {
          ...
          return <div key={p.order_id} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 md:p-5 shadow-lg">
            ...
            <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.025] p-4"><div className="flex items-center gap-2 text-sm font-medium"><FileText size={16} /> Documentation</div><p className="mt-1 text-xs text-muted-foreground">Documentation fee for this property. This is separate from your land purchase balance.</p><div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs text-muted-foreground">Documentation Fee</p><p className="text-lg font-semibold">{p.documentation_price > 0 ? money(p.documentation_price) : 'Not set yet'}</p>{p.documentation_name && <p className="text-xs text-muted-foreground mt-1">{p.documentation_name}</p>}</div><Button variant="outline" onClick={() => startDocumentationPayment(p)} disabled={documentationPayingId === p.order_id}>{documentationPayingId === p.order_id ? <Loader2 className="mr-2 animate-spin" size={16} /> : <CreditCard className="mr-2" size={16} />} Pay Documentation</Button></div></div>
            <div className="mt-5 border-t border-white/10 pt-4"><div className="flex items-center gap-2 text-sm font-medium"><Clock3 size={15} /> Payment Timeline</div><div className="mt-3 space-y-3">{!events.length ? <div className="text-xs text-muted-foreground">No approved payments recorded yet.</div> : events.map((event: any) => <div key={event.event_id} className="flex gap-3 rounded-xl border border-white/10 bg-black/10 p-3"><div className="mt-0.5"><CheckCircle2 size={15} /></div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-medium">{event.event_label}</p><p className="text-sm font-semibold">{money(asNumber(event.amount))}</p></div><p className="mt-1 text-xs text-muted-foreground">{new Date(event.event_at).toLocaleString('en-NG')} • {event.status}</p>{event.reference && <p className="mt-1 truncate text-[11px] text-muted-foreground">Ref: {event.reference}</p>}{event.event_type === 'land' && <p className="mt-1 text-[11px] text-muted-foreground">Balance after payment: {money(asNumber(event.balance_after))}</p>}</div></div>)}</div><div className="mt-4 grid gap-2 text-xs text-muted-foreground"><div className="flex items-center gap-2"><CalendarDays size={14} /> Order created {p.created_at ? new Date(p.created_at).toLocaleDateString('en-NG') : ''}</div><div className="flex items-center gap-2"><CheckCircle2 size={14} /> {balance <= 0 ? 'Property fully paid' : 'Payment plan in progress'}</div></div></div>
          </div>;
        })}
      </CardContent>
    </Card>
