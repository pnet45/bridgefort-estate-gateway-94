...
export default function ProfileCompletionDashboard() {
  ...
  const getStatusColor = () => {
    if (completionPercentage >= 50) return 'text-green-600';
    if (completionPercentage >= 40) return 'text-yellow-600';
    return 'text-red-600';
  };

  const getStatusMessage = () => {
    if (completionPercentage >= 50) return 'Profile Complete - All Features Unlocked!';
      ...
      <CardContent className="pt-6 space-y-6">
          ...
          <div className="relative">
            <svg className="transform -rotate-90 w-40 h-40">
              ...
              <circle
                ...
                className={completionPercentage >= 50 ? 'text-green-600' : 'text-primary'}
                strokeLinecap="round"
              />
            </svg>
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-sm text-muted-foreground">
            <span>0%</span>
            <span className="font-semibold text-yellow-600">50% (Target)</span>
            <span>100%</span>
          </div>
        </div>
        ...
        {/* Missing Fields */}
        {missingFields.length > 0 && completionPercentage < 50 && (
        ...
        <Button
          ...
          className="w-full"
          size="lg"
          variant={completionPercentage >= 50 ? 'outline' : 'default'}
        >
          {completionPercentage >= 50 ? 'View Profile' : 'Complete Your Profile'}
        </Button>
      </CardContent>
    </Card>
  );
