...
// Maps a documentation_types.name to the matching admin-set field on
// estate_doc_pricing, so each estate's admin-configured fee is what the
// client actually pays instead of the flat, estate-agnostic default price.
const DOC_TYPE_TO_PRICING_FIELD: Record<string, keyof Omit<DocPricing, 'estate_id'>> = {
  'Deed of Assignment': 'deed_of_assignment',
  'Survey & Estate Layout': 'survey_plan',
...
const DocumentationPricingTab: React.FC = () => {
  ...
  // Resolves what the client actually pays for a single documentation type
  // on a given estate: the admin's per-estate fee if one has been set,
  // falling back to the generic documentation_types price otherwise.
  const getEstateDocTypePrice = (estateId: string, docType: DocumentationType) => {
    const p = docPricing[estateId];
    const field = DOC_TYPE_TO_PRICING_FIELD[docType.name];
    if (p && field && p[field]) {
  ...
  const handleAddDocumentationToCart = (estate: EstateLite, docType: DocumentationType) => {
    ...
    const price = getEstateDocTypePrice(estate.id, docType);
    addToCart({
    ...
    toast({ title: "Added to Cart", description: `${docType.name} for ${estate.name} (₦${price.toLocaleString()}) added.` });
  };
