...
const ToolbarButton: React.FC<ToolbarButtonProps> = ({ onClick, active, disabled, label, children }) =>
  <Button type="button" variant="ghost" size="sm" onClick={onClick} disabled={disabled} aria-label={label} aria-pressed={active} title={label}
    className={cn('h-8 w-8 p-0 shrink-0', active && 'bg-estate-blue/15 text-estate-blue hover:bg-estate-blue/20')}>{children}</Button>;
...
const EditorToolbar: React.FC<EditorToolbarProps> = ({ editor, onInsertImage }) => {
  ...
  const currentFont = editor.getAttributes('textStyle').fontFamily || SYSTEM_FONT;
  const fontIsSystem = currentFont === SYSTEM_FONT;

  const setHeading = (value: string) => value === 'paragraph'
    ? editor.chain().focus().setParagraph().run()
    : editor.chain().focus().toggleHeading({ level: Number(value) as 1|2|3|4|5|6 }).run();

  // setMark is intentional: it applies the font as a stored mark when the
  // cursor has no selection, so the NEXT characters also use system font.
  const setFontFamily = () => {
  ...
  return <div className="border-b border-border bg-muted/30">
    <div className="flex items-center gap-0.5 p-1.5 overflow-x-auto scrollbar-thin">
      ...
      <ToolbarDivider />
      <Button type="button" variant="ghost" size="sm" onClick={setFontFamily} title="System font" aria-pressed={fontIsSystem}
        className={cn('h-8 shrink-0 gap-1.5 px-2', fontIsSystem && 'bg-estate-blue/15 text-estate-blue hover:bg-estate-blue/20')}>
        <Type className="h-4 w-4" /><span className="text-xs">System font</span>
      </Button>
      <div className="relative shrink-0" title="Font color">
        <Button type="button" variant="ghost" size="sm" onClick={() => colorInputRef.current?.click()} className="h-8 w-8 p-0"><Palette className="h-4 w-4" /></Button>
        <input ref={colorInputRef} type="color" className="absolute inset-0 h-0 w-0 opacity-0" aria-label="Choose font color" onChange={e => editor.chain().focus().setColor(e.target.value).run()} />
      </div>
      ...
      <ToolbarDivider />
      <ToolbarButton label="Insert/edit link (Ctrl+K)" active={editor.isActive('link')} onClick={openLinkPrompt}><Link2 className="h-4 w-4" /></ToolbarButton>
      {editor.isActive('link') && <ToolbarButton label="Remove link" onClick={() => editor.chain().focus().unsetLink().run()}><Link2Off className="h-4 w-4" /></ToolbarButton>}
      <ToolbarDivider />
      <ToolbarButton label="Undo (Ctrl+Z)" disabled={!editor.can().undo()} onClick={() => editor.chain().focus().undo().run()}><Undo2 className="h-4 w-4" /></ToolbarButton>
      <ToolbarButton label="Redo (Ctrl+Y)" disabled={!editor.can().redo()} onClick={() => editor.chain().focus().redo().run()}><Redo2 className="h-4 w-4" /></ToolbarButton>
    </div>
