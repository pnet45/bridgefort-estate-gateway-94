// React Bits — CircularGallery (TS-TW variant)
// Source: https://reactbits.dev/components/circular-gallery
// Adapted: added an `onItemClick` callback so the gallery can drive a fullscreen
...
async function resolveFont(font: string, fontUrl?: string): Promise<string> {
  ...
  try {
    ...
    console.error('CircularGallery3D: unable to load font from', fontUrl, error);
    return font;
  }
}
...
class App {
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    this.moved = Math.max(this.moved, Math.abs(x - this.start), Math.abs(y - this.startY));
  ...
  onKeyDown(e: KeyboardEvent) {
    switch (e.key) {
      ...
      case 'Enter':
      case ' ':
        if (this.onItemClick && this.medias[0]) {
          e.preventDefault();
          const width = this.medias[0].width;
          const rawIndex = Math.round(this.scroll.target / width);
          const itemIndex = ((rawIndex % this.medias.length) + this.medias.length) % this.medias.length;
...
export interface CircularGallery3DProps {
  items?: { image: string; text: string }[];
  bend?: number;
  textColor?: string;
  borderRadius?: number;
...
export default function CircularGallery3D({
  items,
  bend = 3,
  textColor = '#ffffff',
  borderRadius = 0.21,
  ...
  scrollEase = 0.09,
  onItemClick
}: CircularGallery3DProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const itemCountRef = useRef(items?.length ?? 0);
  itemCountRef.current = items?.length ?? 0;
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      app = new App(containerRef.current, {
        ...
        scrollEase,
        onItemClick: index => {
          // Internal list is items.concat(items) for the infinite-scroll illusion;
          // App already reduces the index modulo medias.length, but medias.length
          // is 2x the real item count, so fold it down once more here.
          const count = itemCountRef.current || 1;
          onItemClick?.(index % count);
        }
      });
    });
    return () => {
