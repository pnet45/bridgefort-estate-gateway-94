...
import { CalendarDays, Clock3, MapPin, Sprout, Tractor, Users, ArrowRight, CheckCircle2, Award, Wheat, TrendingUp } from 'lucide-react';
  ...
  if (compact) {
    return (
      <>
          ...
          <div className="container-custom">
            <div className="relative overflow-hidden rounded-[2rem] border border-emerald-900/10 bg-gradient-to-br from-emerald-950 via-emerald-900 to-green-800 p-6 md:p-10 text-white shadow-2xl">
              ...
              <div className="relative grid gap-8 lg:grid-cols-[1.4fr_0.8fr] items-center">
                <div>
                  <Badge className="mb-4 border border-lime-200/30 bg-lime-300/15 text-lime-100 hover:bg-lime-300/20">
                    Featured Training • 25 August 2026
                  </Badge>
                  ...
                  <p className="mt-4 max-w-3xl text-sm leading-7 text-emerald-50/90 md:text-base">
                    A practical seminar and workshop for farmers, intending farmers, entrepreneurs and investors who want to understand how agriculture can become a structured, sustainable and profitable wealth-building venture.
                  </p>
                  ...
                  <div className="mt-7 flex flex-wrap gap-3">
                    <Button onClick={() => setRegistrationOpen(true)} className="bg-lime-300 text-emerald-950 hover:bg-lime-200 font-bold">
                      Register for ATP <ArrowRight className="ml-2 h-4 w-4" />
                    </Button>
                    <Button asChild variant="outline" className="border-white/40 bg-white/5 text-white hover:bg-white hover:text-emerald-950">
                      <a href="/training">View Training Details</a>
                    </Button>
                  </div>
                </div>
                <div className="rounded-3xl border border-white/15 bg-white/10 p-6 backdrop-blur-md">
                  ...
                  <ul className="space-y-3 text-sm text-emerald-50/90">
                    {benefits.slice(0, 5).map((benefit) => <li key={benefit} className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-lime-300" />{benefit}</li>)}
                  </ul>
        ...
        <TrainingRegistrationForm open={registrationOpen} onClose={() => setRegistrationOpen(false)} eventTitle={AGRiculturalTitleForForm()} eventDate={AGRICULTURAL_TRAINING_EVENT_DATE} />
      </>
    );
  }
        ...
        <div className="container-custom relative">
          <div className="mx-auto max-w-6xl">
            <div className="mb-10 text-center">
              ...
              <p className="mx-auto mt-5 max-w-3xl text-base leading-8 text-emerald-50/85 md:text-lg">
                Don't just hear about agriculture. Learn it. Practise it. Invest in it. Profit from it.
              </p>
            </div>
            ...
            <div className="mt-10 grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
              <div className="rounded-3xl border border-white/10 bg-white/10 p-6 md:p-8 backdrop-blur-md">
                ...
                <div className="mt-6 grid gap-3 md:grid-cols-2">
                  {benefits.map((benefit) => <div key={benefit} className="flex gap-3 text-sm leading-6 text-emerald-50/90"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-lime-300" />{benefit}</div>)}
                </div>
              </div>
              <div className="space-y-5">
                <div className="rounded-3xl border border-lime-200/20 bg-lime-300/10 p-6 backdrop-blur-md">
                  <div className="flex items-center gap-3"><Tractor className="h-7 w-7 text-lime-300" /><h2 className="text-xl font-bold">And There's More</h2></div>
                  <p className="mt-4 text-sm leading-7 text-emerald-50/90">Participants will discover how to benefit from <strong>Bridgefort Agrovest Scheme 1 & Scheme 2</strong>, including farmland leasing, crop production, livestock and integrated farming support.</p>
                </div>
                <div className="rounded-3xl border border-white/10 bg-white/10 p-6 backdrop-blur-md">
                  <div className="flex items-center gap-3"><Users className="h-6 w-6 text-lime-300" /><h2 className="text-xl font-bold">Speakers</h2></div>
                  <p className="mt-3 text-sm text-emerald-50/90"><strong>Dr. Dalvin Silva</strong> and other professional agriculturists & experienced farmers.</p>
                  <div className="mt-4 flex items-center gap-2 text-sm text-lime-200"><Award className="h-4 w-4" /> Certificate of Participation</div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-10 rounded-3xl border border-amber-200/20 bg-gradient-to-r from-amber-300/10 to-lime-300/10 p-6 md:p-8">
              <div className="grid gap-5 md:grid-cols-2">
                <div><p className="text-sm font-semibold uppercase tracking-wider text-lime-200">Scheme 1</p><h3 className="mt-1 text-xl font-bold">Farmland for Yearly Lease</h3><p className="mt-2 text-sm text-emerald-50/80">Flexible plot sizes available at affordable rates for crop farming.</p></div>
                <div><p className="text-sm font-semibold uppercase tracking-wider text-lime-200">Scheme 2</p><h3 className="mt-1 text-xl font-bold">Livestock & Integrated Farming Support</h3><p className="mt-2 text-sm text-emerald-50/80">Training, mentorship and continuous support.</p></div>
              </div>
            </div>

            <div className="mt-10 flex flex-col items-center justify-between gap-5 rounded-3xl bg-white p-6 text-center text-emerald-950 md:flex-row md:text-left md:p-8">
              ...
              <Button onClick={() => setRegistrationOpen(true)} size="lg" className="shrink-0 bg-emerald-800 px-7 font-bold text-white hover:bg-emerald-700">Register for ATP <ArrowRight className="ml-2 h-4 w-4" /></Button>
            </div>
          </div>
        </div>
      </section>
...
const AGRiculturalTitleForForm = () => AGRICULTURAL_TRAINING_TITLE;
