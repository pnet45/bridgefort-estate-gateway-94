    ...
    try {
      ...
      if (data.user) {
        ...
        if (isRegisteringAsPBO) {
          ...
          const isAutoGeneratedPbo = !pboCode.trim();
          ...
          if (isAutoGeneratedPbo && data.user?.email) {
            toast({
              title: 'Referral code generated',
              description: `Your PBO referral code is ${finalPbo}. A confirmation email has been sent.`,
            });

            emailService.sendEmail({
              to: data.user.email,
              name: `${firstName} ${lastName}`.trim() || 'BHRealtors Partner',
              subject: 'Your BHRealtors Referral Code',
              body: `Hello ${firstName || 'Partner'},\n\nYour BHRealtors referral code has been generated successfully:\n\n${finalPbo}\n\nShare this code with your referrals so they can credit your network when signing up.\n\nThank you for joining BHRealtors!`,
            }).catch((emailError) => {
              console.error('Failed to send PBO confirmation email:', emailError);
            });
          }
